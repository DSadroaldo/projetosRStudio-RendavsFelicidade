...
# a função 'summary' gera um resumo estatístico descritivo de um objeto
# É um conjunto de medidas e informações que resumem os dados de forma simples, sem precisar olhar cada observação.
summary(data)

# função para gerar descritivas
...
# descritivas de renda
descritivas(Renda)
# descritivas de Felicidade
descritivas(Felicidade)

# >> para entendermos mais as frequencias dotadas dos valores presentes nas descritivas, vou adotar o grafico histograma. Torna-se mais facil de verificarmos estas r frequencias mais didaticas
hist(Felicidade)

# entendendo a estrutura do grafico mediante a função empregada
# - > O código segue a lógica em camadas do ggplot2:   
# - > Define os dados e os eixos → ggplot(data, aes(...))
# - > Adiciona a geometria do gráfico → geom_histogram(...)
# - > Ajusta o tema → theme_bw() e theme(...)
# - > Adiciona títulos e rótulos → labs(...)
x11()
...
# -> percebemos que a distribuição de frequencia da minha variavel Felicidade obedece uma variação de frequencia normal.Isto pq temos uma escalada, não uniforme, de valores dentro dos eixos, assumindo em determinado momento uma parábola, ascendente e descendente, o que é experado.
# -------------------------------------------------------------------------
...
# Distribuição Normal?
# FELICIDADE = SEGUE uma distribuição normal!
# RENDA = NÂO segue uma distruição normal!

# esta informação eh de extrema importância ao que consideramos na escolha do modelo a se utilizar
# Isso pq, para precisamos que nossa variavel resposta siga uma ditribuição de frequencia aparentemente normal
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# CORRELAÇÃO LINEAR
#  - > precisamos validar a relação existente entre as duas variaveis.
#  - > É necessário quantificar essa relação entre as variáveis
#  - > e, este QUANTIFICAR, é quem servirá de baseamento da contrução de nosso modelo.

#  o uso do plot em R, faz exatamente este correlação entre as variaveis.
